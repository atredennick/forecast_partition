...
\noindent{}where the notation $\epsilon_{T+q}^{(X,Y)}$ represents the remaining interactive effect of $X$ and $Y$ on $V_{T+q}^{(F)}$ after accounting for their main effects, such that if the full forecast variance is a function of *X* and *Y*

\begin{align}
V^{(F)}_{T+q} &= V^{(X)}_{T+q} + V^{(Y)}_{T+q} + \epsilon_{T+q}^{(X,Y)}, \\
\epsilon_{T+q}^{(X,Y)} &= V^{(F)}_{T+q} - \left[ V^{(X)}_{T+q} + V^{(Y)}_{T+q}. \right]
\end{align}
